    ...
    <div className='grid gap-2 grid-cols-1 sm:grid-cols-[2fr_1fr] mt-1 rounded-3xl overflow-hidden'>
        <Link to={'/place/photos/'+place._id} className='bg-gray-600 '>
          {place.photos&&place.photos[0] && (
              <img className='aspect-square object-cover w-full h-full' src={"http://localhost:4000/downloads/"+place.photos[0]} />
          )}
        </Link>
        <div className='grid grid-cols-2 sm:grid-cols-1 gap-2'>
          <Link to={'/place/photos/'+place._id}>
          {place.photos&&place.photos[1] && (
            <img className='aspect-square object-cover' src={"http://localhost:4000/downloads/"+place.photos[1]} />
          )}
          </Link>
          <div className=' overflow-hidden relative'>
          {place.photos&&place.photos[2] && (
            <img className='aspect-square object-cover relative ' src={"http://localhost:4000/downloads/"+place.photos[2]} />
          )}
